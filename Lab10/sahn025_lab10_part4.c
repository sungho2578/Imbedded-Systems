...
unsigned char tempSpeaker = 0;
enum Speaker_States {start, on, off} S_state;

//set time
enum SetTime_States {ST_Start, ST_up, ST_upRelease, ST_down, ST_downRelease} ST_state;
...
void Combine_tick()
{
	...
	switch (combine_state)
	{
		...
		break;
		case Combine_run:
		PORTB = blinkLED | threeLED | tempSpeaker;
		break;
		default:
		break;
	}
...
void SetTime_tick()
{
	switch (ST_state)
	{
		...
		{
			ST_state = ST_up;
			speakerTime = (speakerTime > 1) ? speakerTime - 1 : speakerTime;
		}
		else if (~PINA & 0x02)
		{
			ST_state = ST_down;
			speakerTime = (speakerTime < 10) ? speakerTime + 1 : speakerTime;
		}
		else
		ST_state = ST_Start;
		break;
...
int main(void)
{
	unsigned long blinkLED_elapTime = 1000;
	unsigned long threeLED_elapTime = 300;
	unsigned long speaker_elapTime = speakerTime;
	...
	{
		SetTime_tick();
		if (speaker_elapTime >= speakerTime)
		{
			Speaker_tick();
			speaker_elapTime = 0;
		}
